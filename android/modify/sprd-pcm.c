...
#if 1
extern int fytstart;
extern int fyt_debug_audio;
#endif
static unsigned long fyt_irq =0;
static void sprd_pcm_dma_irq_ch(int dma_ch, void *dev_id)
{
	...
	if (!rtd->irq_called) {
		rtd->irq_called = 1;
		sp_asoc_pr_info("IRQ CALL dma_addr:%p offset:%d\n",runtime->dma_addr,rtd->dma_addr_offset);
	}
...
static int f=0;
if (!fyt_debug_audio&&((cur_jiffies -last_jiffies)/HZ)>6) {
   int addr0 = sprd_pcm_dma_get_addr(rtd->uid_cid_map[0],substream);
   int addr1 = sprd_pcm_dma_get_addr(rtd->uid_cid_map[1],substream);
   //int now_pointer1 = sprd_pcm_dma_get_addr(rtd->uid_cid_map[0],substream) -runtime->dma_addr;
   //int now_pointer = sprd_pcm_dma_get_addr(rtd->uid_cid_map[1],substream) -runtime->dma_addr - rtd->dma_addr_offset;
   //printk(KERN_ERR "irq:%ld chn:%d pos0:%4d pos1:%4d \n",fyt_irq,dma_ch,now_pointer1,now_pointer);

   printk(KERN_ERR "irq:%ld chn:%d addr0:%p addr1:%p pos0:%4d pos1:%4d\n",fyt_irq,dma_ch,addr0,addr1,
          addr0-runtime->dma_addr,addr1-runtime->dma_addr - rtd->dma_addr_offset);
   if (f++==6) {
      f =0;
      printk(KERN_ERR "\n");
...
static snd_pcm_uframes_t sprd_pcm_pointer(struct snd_pcm_substream *substream)
{
	...
	struct sprd_runtime_data *rtd = runtime->private_data;
	snd_pcm_uframes_t x;
	int now_pointer,now_pointer1;
	int bytes_of_pointer = 0;
	int shift = 1;
        static int chn1_cross=0;
        static int chn2_cross=0;
	int sel_max = 0;
	struct snd_soc_pcm_runtime *srtd = substream->private_data;
	if (sprd_is_dfm(srtd->cpu_dai) || sprd_is_vaudio(srtd->cpu_dai)) {
	...
	if (rtd->uid_cid_map[0] >= 0) {
		now_pointer1 = sprd_pcm_dma_get_addr(rtd->uid_cid_map[0],
						    substream) -
		    runtime->dma_addr;
		bytes_of_pointer = now_pointer1;
	}
	if (rtd->uid_cid_map[1] >= 0) {
		...
		if (!bytes_of_pointer) {
			bytes_of_pointer = now_pointer;
                        ...
                        if (chn1_cross == 0)
                           chn1_cross = (bytes_of_pointer < rtd->dma_pos_pre[0]);
                        if (chn2_cross == 0)
                           chn2_cross = (now_pointer < rtd->dma_pos_pre[1]);

                        if (chn1_cross==1&&chn2_cross==1) {
                           chn1_cross = 0;
                           chn2_cross = 0;
                        }
                        if (chn1_cross^chn2_cross) {
                           sel_max = 1;
                        }
...
static int last=0;
int i;
if (substream->stream == SNDRV_PCM_STREAM_CAPTURE &&fyt_debug_audio==1&&fytstart) {
 //pointer bytes x frame
   printk(KERN_ERR "------>%d p0:%4d  p1:%4d last:%4d x:%4d \n",f++,now_pointer1,now_pointer,last,x);
   if (0) {
        struct sprd_runtime_data *rtd = substream->runtime->private_data;
        for (i = 0; i < rtd->hw_chan; i++) {
           if (rtd->uid_cid_map[i] >= 0) {
           u32 reg, base_reg;
           u32 offset = sci_dma_dump_reg(rtd->uid_cid_map[i], &base_reg);
           printk(KERN_ERR "error id:%d\n",rtd->uid_cid_map[i]);
           for (reg = base_reg; reg < base_reg + offset; reg += 0x10) {
                   printk(KERN_ERR "0x%04x | 0x%08x 0x%08x 0x%08x 0x%08x\n",
                               (unsigned int)(reg - base_reg),
                               __raw_readl((void __iomem *)(reg + 0x00)),
                               __raw_readl((void __iomem *)(reg + 0x04)),
                               __raw_readl((void __iomem *)(reg + 0x08)),
